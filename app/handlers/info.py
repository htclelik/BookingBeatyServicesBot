...
async def master_info_handler(callback_query: types.CallbackQuery, bot: Bot):
    ...
    if master:
        ...
        #     f"📅Мой график работы: с <b>{master['work_start_hour']}</b> до <b>{master['work_end_hour']}</b> \n выходные: <b>{master['weekend']} </b>\n\n"
        #     f"🏡ВЫ можете увидеть меня на рабочем месте в уютном месте по адресу: <b>{master['address']}</b>\n\n\n"
        #     f"📲Я на связи с вами:\n {master.get('my_telegram', 'Контакт не указан')}\n\n"  # Используем .get() для безопасного доступа
        #     f"📩Можете написать мне на почту:\n {master.get('email', 'Не указан')}\n\n"
        #     f"📢Канал в Telegram:\n 👉{master.get('cl_telegram', 'Не указан')}\n\n"
        #     f"🌍Вы можете ознакомиться с <b>моими работами</b> \n 👉группа в контакте VK:\n {master.get('vk_club', 'Не указан')}\n\n"
        # )
        about_me_text = (
            f"{master['what your services mean']}.\n\n"
            f"🪩 Моя область работы: {master['work_area']}\n"
